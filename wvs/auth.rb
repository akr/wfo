...
def (WVS::Auth).apache_authtypekey_handler(webclient, uri, req, resp)
  errpage = resp.body
...
def (WVS::Auth).typekey_login(webclient, typekey_uri)
  ...
  KeyRing.with_authinfo(KeyRing.typekey_protection_domain) {|username, password|
    ...
    typekey_login_form.make_request {|req|
      resp = webclient.do_request_cookie(typekey_login_form.action_uri, req)
    }
  }
  ...
  if resp.code == '200' # send email address or not?
    ...
    resp = webclient.do_request_cookie(email_form.action_uri, req)
  end
